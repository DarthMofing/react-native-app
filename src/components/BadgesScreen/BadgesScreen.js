...
import { View, ActivityIndicator, StyleSheet, Text, FlatList, Alert, StatusBar } from 'react-native'
...
class BadgesScreen extends React.Component {
  ...
  handlePress = item => {
    this.props.navigation.navigate('BadgesDetails', { item })
  }
  ...
  handleDelete = item => {
    Alert.alert('Are you sure?',
      `Do you want to delete ${item.name}'s badge?\n\nThis process cannot be reverted.`,
      [
        {
          text:'Cancel',
          style:'cancel',
    ...
    return (
      <View style={[styles.container, styles.horizontal]}>
        ...
        <FlatList
          ...
          data={badges}
          renderItem={({ item }) => (
            <BadgesItem key={item._id} item={item} onPress={() => this.handlePress(item)}
